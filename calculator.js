...
/* ── calculator-core (inlined so no separate file is required) ── */
(function initAurumCalculatorCore(root) {
  ...
  function simulateProjection(options) {
    ...
    let balance = initial, simplePrincipal = initial, profitPool = 0, isCompound = startOn;
    let switchCount = 0, compoundDays = 0, simpleDays = 0, totalProfit = 0, totalDeposits = 0;
    const usedOffsets = [], rows = [];
    for (let day = 1; day <= numDays; day += 1) {
      ...
      if (switched) {
        ...
        isCompound = newMode; switchCount += 1;
      }
      const depositToday = normalizeAmount(depositMap[day]);
      if (depositToday > 0) {
        totalDeposits += depositToday;
        if (isCompound) balance += depositToday; else simplePrincipal += depositToday;
      }
      ...
      let endBal;
      if (isCompound) { compoundDays += 1; balance += profit; endBal = balance; }
      else { simpleDays += 1; profitPool += profit; endBal = simplePrincipal; }
      totalProfit += profit;
      rows.push({ day, pkgKey: pkg.key, pkgLabel: pkg.label, start: principal, profit, end: endBal,
        rate: Number(pkg.daily || 0) * offset, isCompound, switched, deposit: depositToday,
        deposited: depositToday > 0, activeValue: endBal, withdrawnProfit: profitPool,
        projectedValue: endBal + profitPool });
    }
    return { rows, offsets: usedOffsets, finalValue: rows.length ? rows[rows.length - 1].projectedValue : initial,
      totalProfit, totalDeposits, switchCount, compoundDays, simpleDays };
  }
